...
async function escolherTrechos(transcript) {
  ...
  const prompt = `
Você é um editor de vídeos virais. Abaixo está a transcrição de um vídeo
longo, com o tempo (em segundos) de cada palavra.

Identifique de 4 a 6 trechos com potencial viral (histórias completas,
momentos de impacto, dados surpreendentes, virada de emoção). Cada trecho
deve ter entre 30 e 75 segundos.

Responda APENAS em JSON, neste formato exato, sem texto antes ou depois:
[{"inicio": 12.5, "fim": 58.2, "titulo": "Título curto e chamativo"}]

Transcrição:
${textoComTempo.slice(0, 30000)}
`.trim();
...
app.post("/process", async (req, res) => {
  ...
  if (!youtubeUrl) return res.status(400).json({ error: "youtubeUrl é obrigatório" });
  ...
  await supabase.from("jobs").insert({ id: jobId, status: "baixando", youtube_url: youtubeUrl });
  ...
  try {
    ...
    await supabase.from("jobs").update({ status: "transcrevendo" }).eq("id", jobId);

    const transcript = await transcrever(videoPath);
    await supabase.from("jobs").update({ status: "selecionando_trechos" }).eq("id", jobId);
    ...
    for (let i = 0; i < trechos.length; i++) {
      const clipePath = await cortarClipe(videoPath, trechos[i], jobDir, i, transcript);
